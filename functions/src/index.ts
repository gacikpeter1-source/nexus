...
interface RawBoxscoreEvent {
  ...
  teamFullName: string;
  // goal
  scorer?: { number: string; name: string };
  assists?: { number: string; name: string }[];
  // penalty
  player?: { number: string; name: string };
  infraction?: string;
  minutes?: number;
}
 ...
 * NOT duplicated.
 */
function parseHlcanaBoxscore($: cheerio.CheerioAPI): RawBoxscoreEvent[] {
  ...
  $(`${periodSel}, ${eventSel}`).each((_, el) => {
    ...
    if (evType !== 'GÓL' && evType !== 'TREST') return;
    ...
    if (evType === 'GÓL') {
      const scorers: { label: string; number: string; name: string }[] = [];
 ...
 */
async function runBoxscoreSyncForTeam(clubId: string, teamId: string): Promise<{ processed: number; reviewsGenerated: number }> {
  const clubSnap = await db.collection('clubs').doc(clubId).get();
  if (!clubSnap.exists) throw new HttpsError('not-found', 'Club not found.');
  const club = clubSnap.data()!;

  const config = club['leagueScraperConfigs']?.[teamId];
  const teamIdentifier = config?.['teamIdentifier'];
  const scraperUrl = config?.['url'];
  ...
  const detailUrlByScrapedId = new Map<string, string>();
  try {
    const freshGames = await scrapeGamesFromUrl(scraperUrl);
    for (const g of freshGames) {
      if (g.detailUrl) detailUrlByScrapedId.set(g.externalId, g.detailUrl);
    }
  } catch (err) {
    logger.error(`runBoxscoreSyncForTeam: re-scrape failed for ${scraperUrl}`, err);
  }
  ...
  for (const gameDoc of gamesSnap.docs) {
    ...
    if (data['boxscoreStatus']) continue;

    let detailUrl: string | undefined = data['detailUrl'];
    if (!detailUrl && data['scrapedId']) {
      detailUrl = detailUrlByScrapedId.get(data['scrapedId']);
      if (detailUrl) await gameDoc.ref.update({ detailUrl });
    }
  ...
  logger.log(`runBoxscoreSyncForTeam: club ${clubId} team ${teamId} — ${processed} checked, ${reviewsGenerated} reviews generated`);
    ...
    for (const [teamId, config] of Object.entries(configs)) {
      ...
      try {
        ...
        for (const scrapedGame of scrapedGames) {
          ...
          const status: 'upcoming' | 'played' =
            new Date() > zonedTimeToUtc(isoDate, '23:59', EVENT_TIMEZONE) ? 'played' : 'upcoming';
...
export const sendRegistrationInviteEmail = onDocumentCreated(
  ...
  async (event) => {
    ...
    const deadline: Date | null = registration['deadline']
      ? zonedTimeToUtc(registration['deadline'] as string, '23:59', EVENT_TIMEZONE)
      : null;
...
export const sendTournamentRegistrationReminders = onSchedule('0 8 * * *', async () => {
  ...
  for (const registrationDoc of registrationsSnap.docs) {
    ...
    const deadline = zonedTimeToUtc(registration['deadline'] as string, '23:59', EVENT_TIMEZONE);
    if (deadline <= now || deadline > in24h) continue;
...
async function notifyWaitlistInvite(eventId: string, event: FirebaseFirestore.DocumentData, userId: string, expiresAt: string, isGoalie = false): Promise<void> {
...
interface WaitlistTrack {
  limitField: 'participantLimit' | 'goalieLimit';
  confirmedField: 'confirmedCount' | 'confirmedGoalieCount';
  waitlistField: 'waitlist' | 'goalieWaitlist';
  pendingInviteField: 'pendingInvite' | 'goaliePendingInvite';
  isGoalie: boolean;
}

const GENERAL_TRACK: WaitlistTrack = {
...
const GOALIE_TRACK: WaitlistTrack = {
 ...
 */
async function tryPromoteWaitlistTrack(
  ...
  const waitlist: string[] = Array.isArray(data[track.waitlistField]) ? data[track.waitlistField] : [];
  const openSlots = limit - confirmed - (hasPendingInvite ? 1 : 0);
  if (openSlots <= 0 || hasPendingInvite || waitlist.length === 0) return null;

  return db.runTransaction(async (tx) => {
    const snap = await tx.get(eventRef);
    const fresh = snap.data();
    if (!fresh) return null;
    const freshWaitlist: string[] = Array.isArray(fresh[track.waitlistField]) ? fresh[track.waitlistField] : [];
    if (fresh[track.pendingInviteField] || freshWaitlist.length === 0) return null; // already handled by a concurrent run
...
export const promoteFromEventWaitlist = onDocumentWritten(
  ...
  async (event) => {
    ...
    if (generalInvite) {
      ...
      logger.log(`promoteFromEventWaitlist: invited ${generalInvite.userId} for event ${eventId}, expires ${generalInvite.expiresAt}`);
    }
    ...
    if (goalieInvite) {
      ...
      logger.log(`promoteFromEventWaitlist: invited goalie ${goalieInvite.userId} for event ${eventId}, expires ${goalieInvite.expiresAt}`);
    }
  }
);

async function expireTrackInvites(track: WaitlistTrack): Promise<void> {
  ...
  for (const docSnap of snap.docs) {
    ...
    const expiredUserId = await db.runTransaction(async (tx) => {
      ...
      const pendingInvite = data?.[track.pendingInviteField];
      if (!pendingInvite || pendingInvite.expiresAt > nowIso) return null; // already answered or refreshed
      const userId = pendingInvite.userId as string;
      const waitlist: string[] = Array.isArray(data?.[track.waitlistField]) ? data[track.waitlistField] : [];
      tx.update(eventRef, {
    ...
    if (expiredUserId) {
      logger.log(`expireEventWaitlistInvites: ${track.isGoalie ? 'goalie ' : ''}invite for ${expiredUserId} lapsed on event ${docSnap.id}, requeued`);
    }
  }
}
