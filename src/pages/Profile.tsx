...
export default function Profile() {
  ...
  function triggerChildPhotoUpload(childId: string) {
    pendingChildPhotoId.current = childId;
    childFileInputRef.current?.click();
  }

  async function handleChildPhotoUpload(event: React.ChangeEvent<HTMLInputElement>) {
    const file = event.target.files?.[0];
    ...
    try {
      ...
      setChildren(prev => prev.map(c => c.id === childId ? { ...c, photoURL: uploadResult.downloadUrl } : c));
    } catch (error) {
      console.error('Error uploading child photo:', error);
      alert(t('profile.photo.uploadError'));
    } finally {
      setUploadingChildId(null);
      pendingChildPhotoId.current = null;
    }
  }
      ...
      <div className="space-y-6">
        ...
        {user.role !== 'admin' && isParentEnabled && (
          <div className="bg-app-card border border-white/10 rounded-2xl shadow-card p-6">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3">
                        <button
                          onClick={() => triggerChildPhotoUpload(child.id)}
                          disabled={uploadingChildId === child.id}
                          className="relative group w-10 h-10 rounded-full flex-shrink-0"
                          title={t('parent.changePhoto')}
                        >
                          {child.photoURL ? (
                            <img
                              src={child.photoURL}
            ...
            {/* Shared hidden file input for all child avatar uploads */}
            <input
